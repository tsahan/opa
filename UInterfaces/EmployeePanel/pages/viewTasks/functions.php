...
function get_tasks(){
	$id=$_GET['id'];
	global $db;
	$query= "SELECT taskID, taskTitle, startDate, dueDate FROM task WHERE members LIKE'%$id%'";
...
function get_events(){
	$id=$_GET['id'];
	global $db;
	$query= "SELECT eventsID, eventsTitle, eventsDate FROM events WHERE users LIKE'%$id%' OR users ='ALL'";
	$res = mysqli_query($db, $query);
	return mysqli_fetch_all($res, MYSQLI_ASSOC);
}
...
function get_meetings(){
	$id=$_GET['id'];
	global $db;
	$query= "SELECT meetingsID, meetingsTitle, meetingsDate, meetingsTime FROM meetings WHERE users LIKE'%$id%'";
	$res = mysqli_query($db, $query);
	return mysqli_fetch_all($res, MYSQLI_ASSOC);
}
